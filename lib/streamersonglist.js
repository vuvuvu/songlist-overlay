...
function displayCurrentSong(song) {
  ...
  const requesterName = (song.requests && song.requests[0] && song.requests[0].name) ? song.requests[0].name : 'Anonymous';
  container.html(`
    <div class="current-song">
      <div class="song-position current">${song.position}</div>
      <div class="current-song-info">
        <div class="song-title current">${song.song.title || 'Unknown'}</div>
        <div class="song-artist current">by ${song.song.artist || 'Unknown Artist'}</div>
        <div class="song-requester current">Requested by: ${requesterName}</div>
      </div>
    </div>
  `);
}
        ...
        const songDiv = $(`
          <div class="cycling-song fade-in">
            ...
            <div class="queue-song-info">
              ...
              <div class="song-artist queue">by ${song.song.artist || 'Unknown Artist'}</div>
              <div class="song-requester queue">Requested by: ${requesterName}</div>
            </div>
          </div>
        `);
